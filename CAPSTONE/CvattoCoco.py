...
class CVATtoCOCO:
    ...
    def __init__(self, data_dir, split_test_size=0.10):
        ...
        for idx, d in enumerate(os.listdir(data_dir)):
            ...
            os.makedirs(self.outdir + self.traindir + d, exist_ok=True)
            os.makedirs(self.outdir + self.valdir + d,exist_ok=True)

            # read the json file
            # print(self.rootdir + d + '/coco.json')
            f = open(self.rootdir + '/' +  d + '/coco.json')
            labels = json.load(f)
            cats= self.parse_coco_dataset(labels, d, idx)
            f.close()
    ...
    def parse_coco_dataset(self, json_input, d, cat_id):
        ...
        for img in json_input['images']:
            ...
            if img_id in X_train:
                ...
                shutil.move(file_path, self.outdir + self.traindir + d + '/')
                ...
                shutil.move(file_path, self.outdir + self.valdir + d + '/')
...
# dataset = CVATtoCOCO('/home/neha/Work/eva6/capstone/data_subset/')
# print("DONE")
